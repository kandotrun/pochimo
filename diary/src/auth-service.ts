...
export class AuthService {
  ...
  listHouseholdUsers(householdId) {
    return this.db
      .prepare("SELECT id, username FROM users WHERE household_id = ? ORDER BY id ASC")
      .all(Number(householdId));
  }
  ...
  createUserWithInvite({ username, password, inviteCode }) {
    ...
    this.db
      .prepare("UPDATE invites SET used_by = ?, used_at = CURRENT_TIMESTAMP WHERE code = ?")
      .run(user.id, invite.code);
    return user;
  }
  ...
  #createUser({ username, password, householdId = null }) {
    ...
    const result = this.db
      .prepare("INSERT INTO users (username, password_hash, household_id) VALUES (?, ?, ?)")
      .run(username.trim(), passwordHash, householdId);
  ...
  createEmailLoginCode({ email, inviteCode = "" }) {
    ...
    this.db
      .prepare("INSERT INTO login_codes (email, code_hash, invite_code, expires_at) VALUES (?, ?, ?, ?)")
  ...
  verifyEmailLoginCode({ email, code, inviteCode = "" }) {
    ...
    const loginCode = rows.find(
      (row) => String(row.inviteCode || "") === normalizedInvite && verifyPassword(code, row.codeHash),
    );
    ...
    let user = this.db
      .prepare("SELECT id, username, household_id AS householdId FROM users WHERE username = ?")
      .get(normalizedEmail);
    if (!user) {
      if (!this.hasUsers()) {
  ...
  login({ username, password }) {
    ...
    const user = this.db
      .prepare("SELECT id, username, password_hash, household_id AS householdId FROM users WHERE username = ?")
      .get(username.trim());
  ...
  getPetProfile(userId, householdId = userId) {
    const own = this.db
      .prepare("SELECT name, photo, updated_at AS updatedAt FROM pet_profiles WHERE user_id = ?")
      .get(userId);
  ...
  saveHouseholdSettings(householdId: number, patch: { mischiefEmailEnabled?: boolean } = {}) {
    const current = this.getHouseholdSettings(householdId);
    const mischiefEmailEnabled =
      patch.mischiefEmailEnabled == null ? current.mischiefEmailEnabled : Boolean(patch.mischiefEmailEnabled);
...
export function parseCookies(req: { headers: { cookie?: string } }): Record<string, string> {
  ...
  for (const part of header.split(";")) {
    ...
    if (index === -1) continue;
    entries.push([part.slice(0, index).trim(), decodeURIComponent(part.slice(index + 1).trim())]);
  }
...
function genericSkippedLoginCode(email: string) {
  return {
    email,
    code: "",
