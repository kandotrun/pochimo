...
notify=true は、イタズラ疑い・危険そうな状態・すぐ確認した方がよい異常に限定してください。
暗い、電気が消えた、見えない、判定困難、普段通りの休憩、ご飯・水・トイレだけでは notify=false にしてください。`;
}
...
- 画像が暗い/判定困難なら正直に書く
- Markdownのみ返す
- 見出しは「今日の様子」「気になる点」「明日見ること」の3つだけにする
- モデル名、解析枚数、内部処理などの技術メモは書かない
